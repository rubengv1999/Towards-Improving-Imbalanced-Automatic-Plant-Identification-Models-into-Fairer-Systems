...
images = {}
for id in speciesId:
  images[id] = sum([1 for fileName in os.listdir(basePath + id) if (fileName.endswith('.jpg') or fileName.endswith('.JPG'))])
...
class PlantsDataset(torch.utils.data.Dataset): 
  def __init__(self, transform, speciesCount): 
    ...
    for i, folder in enumerate(self.folders):
        a = [x for x in os.listdir(self.basePath + "/" + folder) if (x.endswith(".jpg") or x.endswith(".JPG"))]
        if len(a) < 10:
            self.images[folder] = a
  ...
  def __getitem__(self, index): 
    for i in range(self.speciesCount):
      ...
      if index < 0:
        ...
        imagePath = os.path.join(self.basePath + str(self.folders[i]), imageName)
...
class DataModule(pl.LightningDataModule):
  def __init__(self, dataset, batch_size, test_size, species_count, class_weigths, sampling, data_transforms, use_index):
    super().__init__()
    ...
    self.idxPATH = 'idxPATH/' + str(self.dataset) + '_minority.pkl'
  ...
  def prepare_data(self):
    ...
    if self.use_index:
      ...
      self.train_idx, self.val_idx = checkpoint["train_idx"], checkpoint["val_idx"]
      a_file.close()
    else:
      num_samples = self.train_dataset.__len__()
      self.train_idx, self.val_idx = train_test_split(np.arange(num_samples), test_size=self.test_size, stratify=self.train_dataset.targets)
    ...
    if self.class_weigths:
      ...
      self.class_weights = torch.tensor(class_weight.compute_class_weight(class_weight='balanced', classes=unique, y=targets),dtype=torch.float)
      ...
      self.train_dataset = torch.utils.data.Subset(self.train_dataset, self.train_idx)
      ...
      self.train_sampler = WeightedRandomSampler(weights=samples_weight.type('torch.DoubleTensor'), num_samples=len(samples_weight))

  def train_dataloader(self):
    return DataLoader(self.train_dataset, batch_size= self.batch_size, shuffle = False, sampler=self.train_sampler, num_workers =8, persistent_workers=True)

  def val_dataloader(self):
    return DataLoader(self.val_dataset, batch_size= self.batch_size, shuffle = False, sampler=self.val_sampler, num_workers =8, persistent_workers=True)
...
class DataModuleSelfSupervision(pl.LightningDataModule):
  ...
  def train_dataloader(self):
    return DataLoader(self.dataset, batch_size= self.batch_size, shuffle = True, drop_last=True, num_workers =8, persistent_workers=True)
...
class Resnet50SelfSupervision(nn.Module):
  def __init__(self, out_features):
    ...
    self.backbone.fc = nn.Sequential(nn.Linear(dim_mlp, dim_mlp), nn.ReLU(), nn.Linear(2048, out_features))
...
class ModeloSelfSupervision(pl.LightningModule):
  ...
  def training_step(self, batch, batch_idx):
    ...
    self.log("train/general_acc", accuracy_score(labels, preds), on_step=True, on_epoch=True)
    return loss
    
  def configure_optimizers(self):
    optimizer = torch.optim.Adam(self.model.parameters(), lr=0.0003, weight_decay=1e-4, eps=1e-4)
...
class ModeloBase(pl.LightningModule):
  ...
  def training_step(self, batch, batch_idx):
    ...
    self.log("train/loss", loss, on_step=True, on_epoch=True)
    self.log("train/general_acc", accuracy_score(ys, preds), on_step=True, on_epoch=True)
  ...
  def validation_step(self, batch, batch_idx):
    ...
    self.log("val/general_acc", accuracy_score(ys, preds), on_step=True, on_epoch=True)
  ...
  def test_step(self, batch, batch_idx):
    ...
    self.log("test/general_acc", accuracy_score(ys, preds), on_step=False, on_epoch=True)
  ...
  def metrics(self, ys, preds, phase):
    ...
    #self.log(f"{phase}/balanced_acc_minority", balanced_accuracy_score(ys_minority[0], preds_minority[0]))

    self.log(f"{phase}/balanced_acc_minority", balanced_accuracy_score(ys, preds))
  ...
  def correlation(self, ys, preds, phase):
    ...
    plt.ylabel('Accuracy')
    self.logger.experiment.log({f"{phase}/acc_corr":[wandb.Image(plot, caption=f"Acc : {round(np.sum(cm.diagonal()) / np.sum(cm),3)}, Pearson: {round(pearsonr(x, acc)[0],3)}, Spearman: {round(spearmanr(x, acc)[0], 3)}, Kendall: {round(kendalltau(x, acc)[0], 3)}")]})
...
SPECIES_COUNT = 254 if DATASET == Datasets.CRLeaves else 4196#10000
TEST_SIZE = 0.5
BATCH_SIZE = 256
USE_INDEX = False
ID = None
